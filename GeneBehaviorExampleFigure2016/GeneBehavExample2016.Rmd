...
We keep only expression values of the genes of interest. 
...
```{r}
PmaipPlot <- ggplot(Pmaip, aes(x = factor(Pmaip$ProlifIndex, levels = rev(levels(factor(Pmaip$ProlifIndex)))), y = log2(Pmaip$median), group = Pmaip$all, color = Pmaip$CellType)) + geom_line(aes(linetype = factor(Pmaip$Replicate))) + geom_point() + geom_errorbar(width = 0.05, aes(ymin = log2(Pmaip$lowerCI), ymax = log2(Pmaip$higherCI))) + scale_color_manual(values = c("#538135", "#903B6F")) + ylab("log2(TPM)") + xlab("") + theme_bw() + theme(legend.title = element_blank(), plot.title = element_text(hjust = 0.5)) + ggtitle("Pmaip1")
PmaipPlot
ggsave(PmaipPlot, filename = "Pmaip1_2016.pdf", width = 4, height = 4)
```
...
TarsFIB16M1 <- FIB2016[FIB2016$X == "ENSMUSG00000022241", Columns2Keep2016M1]
...
TarsESC16M2 <- ESC2016[ESC2016$X == "ENSMUSG00000022241", Columns2Keep2016M2]
TarsESC16F2 <- ESC2016[ESC2016$X == "ENSMUSG00000022241", Columns2Keep2016F2]
colnames(TarsESC16S1) <- NewColNames
colnames(TarsESC16M1) <- NewColNames
colnames(TarsESC16F1) <- NewColNames
colnames(TarsESC16S2) <- NewColNames
colnames(TarsESC16M2) <- NewColNames
colnames(TarsESC16F2) <- NewColNames

#Build a dataframe to plot. 

Tars <- smartbind(TarsFIB16S1, TarsFIB16M1, TarsFIB16F1, TarsFIB16S2, TarsFIB16M2, TarsFIB16F2, TarsESC16S1, TarsESC16M1, TarsESC16F1, TarsESC16S2, TarsESC16M2, TarsESC16F2)
...
```{r}
TarsPlot <- ggplot(Tars, aes(x = factor(Tars$ProlifIndex, levels = rev(levels(factor(Tars$ProlifIndex)))), y = log2(Tars$median), group = Tars$all, color = Tars$CellType)) + geom_line(aes(linetype = factor(Tars$Replicate))) + geom_point() + geom_errorbar(width = 0.05, aes(ymin = log2(Tars$lowerCI), ymax = log2(Tars$higherCI))) + scale_color_manual(values = c("#538135", "#903B6F")) + ylab("log2(TPM)") + xlab("") + theme_bw() + theme(legend.title = element_blank(), plot.title = element_text(hjust = 0.5)) + ggtitle("Tars")
TarsPlot
ggsave(TarsPlot, filename = "Tars_2016.pdf", width = 4, height = 4)
```
...
Tnfrsf21Plot
ggsave(Tnfrsf21Plot, filename = "Tnfrsf21_2016.pdf", width = 4, height = 4)
```